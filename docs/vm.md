...
The VM features the following applications:

* EPICS-base 7.0.8 in the folder /opt/epics 
* most common support modules including asyn, streamDevice, AreaDetector, the whole list of the support modules can be found in the /opt/epics 
* bluesky
* EPICS middle layer services including Phoebus, Archiver Appliance, Alarm-Server, Kafka, Zookeper (config can be accessed from ~/.epics-tools)

## EPICS and other middle layer services
...
In case of issues, you can try to run the kafka-setup image once again.
...
`
docker logs kafka/zookeeper/alarm-server
`

Those logs can provide valuable information on the state of the microservices.
...
2. Download [Ubuntu](https://drive.google.com/file/d/1OAqo3Mb5vQ5wAsBDxOoQVmFxTBeoZFCI/view?usp=drive_link) and import it to the VirtualBox
3. Password is `epics123`
...
3. Use `docker-compose start` (if the containers are in a paused state) or `docker-compose up -d`
